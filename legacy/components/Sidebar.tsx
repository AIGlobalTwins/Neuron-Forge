...
export function Sidebar({ lang, toggleLang, hasKey, onHistory, onDocs, onSettings, onHome, t }: Props) {
  ...
  const item = "flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-gray-400 hover:text-white hover:bg-white/[0.05] transition-colors text-sm";
  ...
  return (
    <aside className={`${collapsed ? "w-16" : "w-60"} shrink-0 border-r border-[#1e1e1e] bg-[#0a0a0a]/95 backdrop-blur-sm flex flex-col h-screen sticky top-0 transition-[width] duration-200`}>
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        <button onClick={goClients} className={`${item} ${collapsed ? "justify-center px-0" : ""}`} title="Clients">
        ...
        <button onClick={onHistory} className={`${item} ${collapsed ? "justify-center px-0" : ""}`} title={t.history}>
          <svg viewBox="0 0 20 20" className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button onClick={onSettings} className={`${item} ${collapsed ? "justify-center px-0" : ""}`} title={t.settings}>
          <svg viewBox="0 0 20 20" className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button onClick={onDocs} className={`${item} ${collapsed ? "justify-center px-0" : ""}`} title={t.docs}>
          <svg viewBox="0 0 20 20" className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
            <path d="M4 4h12v12H4z" /><path d="M7 8h6M7 11h4" />
          </svg>
          {!collapsed && <span>{t.docs}</span>}
        </button>
      </nav>
