...
export function ConsultingModal({ onClose, onOpenTool }: Props) {
  ...
  async function handleBuildPlan() {
    ...
    try {
      const res = await fetch("/api/consulting/plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ area, problem, questions, answers, clientProfile: activeClient ?? null }),
      });
      const data = await safeJson(res);
      if (!res.ok) throw new Error(data.error);
      setPlan(data.plan);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto fade-up">
      <div className="w-full max-w-4xl bg-[#0d0d0d] border border-[#1e1e1e] rounded-2xl overflow-hidden shadow-2xl flex flex-col">
          ...
          {step === "generating" && (
            <div className="px-6 py-16 flex flex-col items-center gap-6">
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-gray-600 text-xs">AI is analyzing the data...</p>
              </div>
