        ...
        if(waEls.length){
          waEls.forEach(function(el){
            if(el.tagName==='A'){
              ...
              if(!/wa\\.me\\/\\d{6,}/i.test(h))el.setAttribute('href',WA); // only repair broken/placeholder links; keep an already-valid (e.g. Integrations-configured) number
            }
...
/** Remove a previously-injected site-guard block, so an AI edit can re-apply a fresh one. */
export function stripSiteGuard(html: string): string {
  return html
    .replace(/<!-- nf:guard:start -->[\s\S]*?<!-- nf:guard:end -->/g, "")
    .replace(/\n{3,}/g, "\n\n");
}
